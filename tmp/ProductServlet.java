...
public class ProductServlet extends HttpServlet {
	...
	private final String C045_SEARCH = JSP_HOME+"C045_Search.jsp";
	
	/**
	 * doGetメソッド
	 * セッションスコープの存在を確認し、存在しない場合はログイン画面へ、存在する場合はdoService（アクションメソッド）へと遷移する。
	...
	/**
	 * doPostメソッド
	 * セッションスコープが存在するか確認し、存在しない場合は、ログイン画面からの要求であった場合のみdoService（アクションメソッド
	...
	public void doService(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		//	カタログ情報用Bean
		CatalogBean cb=(CatalogBean) this.getServletContext().getAttribute("catalogBean");
	
		if(cb==null) {
			cb=new CatalogBean();
			this.getServletContext().setAttribute("catalogBean", cb);
		}
		...
		try {
			...
			//	 メイン画面にて照会ボタン押下時
				...
				//		 照会キーの値をBeanに格納
				String key=request.getParameter("a020_productID");
				pib.setProductID(request.getParameter("a020_productID"));
				//	 遷移元の画面IDを変数へ格納
				previous_view  = A020_MAIN;
				...
				try {
					...
					pibList.add(pib);
					pib.setSearchKey("商品ID："+pib.getProductID());
					dispatcher = request.getRequestDispatcher(C040_PRODUCT_REFER);
				}catch(ProductException pe) {
					if(pe.getMessage().equals("登録のない商品IDです。正しいIDを入力してください。")) {
						referLogic.productInfoSearch(pibList,key);
						pibList.get(0).setSearchKey(key);
						dispatcher = request.getRequestDispatcher(C040_PRODUCT_REFER);
					}else {
						throw pe;
					}
...
//	---------------------------------------------------------------------------------
//			 メイン画面にて詳細照会に移動
			else if (request.getParameter("a020_search") != null) {
				previous_view = A020_MAIN;
...
//			カタログのドロップダウンリストを更新
				...
				dispatcher = request.getRequestDispatcher(C045_SEARCH);
			} 
//			詳細照会画面から照会をクリック
			else if(request.getParameter("c045_search") != null) {
				previous_view = C045_SEARCH;
				ReferLogic referLogic = new ReferLogic();

				String type = request.getParameter("xxxx_productType");
				String name = request.getParameter("xxxx_productName");
				String description = request.getParameter("xxxx_description");

				referLogic.productInfoSearch(pibList, type, name, description);
				...
				if(!description.equals("")) {
					k+="商品説明："+description+"&emsp;";
				}

				pibList.get(0).setSearchKey(k);
				dispatcher = request.getRequestDispatcher(C040_PRODUCT_REFER);
			}
			
			
//			詳細照会画面から更新をクリック
			else if ( request.getParameter("c040_update") != null ) {
				//	 商品IDの値をBeanに格納
				pib.setProductID(request.getParameter("c040_productID"));
				//	 遷移元の画面IDを変数へ格納
				previous_view  = A020_MAIN;
				//	 商品存在確認処理
				FindProductLogic findProductLogic = new FindProductLogic();
...
//			 メイン画面にて照会ボタン押下時
			else if ( request.getParameter("c040_delete") != null ) {
				//	 商品IDの値をBeanに格納
				pib.setProductID(request.getParameter("c040_productID"));
				//	 遷移元の画面IDを変数へ格納
				previous_view  = A020_MAIN;
...
//	------------------------------------------------------------------------------------
			...
			//	 ログアウトボタン押下時
			} else if ( request.getParameter("a020_logout") != null ||
						request.getParameter("c010_logout") != null ||
						request.getParameter("c015_logout") != null ||
						request.getParameter("c020_logout") != null ||
						request.getParameter("c025_logout") != null ||
						request.getParameter("c030_logout") != null ||
						request.getParameter("c035_logout") != null ||
						request.getParameter("c040_logout") != null ||
						request.getParameter("c045_logout") != null) {
				HttpSession session = request.getSession(false);
				//	 ユーザIDセッションを削除
			...
			//	 メイン画面に戻るが押下された場合
			} else if ( request.getParameter("c010_return") != null ||
						request.getParameter("c015_return") != null ||
						request.getParameter("c020_return") != null ||
						request.getParameter("c025_return") != null ||
						request.getParameter("c030_return") != null ||
						request.getParameter("c035_return") != null ||
						request.getParameter("c040_return") != null ||
						request.getParameter("c045_return") != null) {
				//	 メイン画面用エラーメッセージ初期化処理
				pib.reset();
				//	 遷移先にメイン画面を指定
				dispatcher = request.getRequestDispatcher(A020_MAIN);
			...
			request.setAttribute("userInfoBean", uib);
			request.setAttribute("productInfoBean", pib);
			request.setAttribute("catalogBean", cb);
